...
// (in the stp-pokeemerald repo) every time the ROM is rebuilt. Entries
...
const PATCH_VERSIONS = {
  ...
  "by_version": {
    "10502": {
      ...
      "lookups": {
        "stp_vars": "0x0203CF64",
        "stp_callback1": "0x0203CF74",
        "stp_callback2": "0x0203CF78",
        "stp_intro_species": "0x0203CF7C",
        "stp_starter_0": "0x0203CF7E",
        "stp_starter_1": "0x0203CF80",
        "stp_starter_2": "0x0203CF82",
        "stp_tracking": "0x0203CF84"
      },
      "fingerprint": [
        {
          "addr": "0x03002710",
    ...
    "10601": {
      ...
      "lookups": {
        "stp_vars": "0x0203CF64",
        "stp_callback1": "0x0203CF90",
        "stp_callback2": "0x0203CF94",
        "stp_intro_species": "0x0203CF98",
        "stp_starter_0": "0x0203CF9A",
        "stp_starter_1": "0x0203CF9C",
        "stp_starter_2": "0x0203CF9E",
        "stp_tracking": "0x0203CFA0",
        "stp_textbox_open": "0x0203CFA4"
      },
      "fingerprint": [
        {
          "addr": "0x03002710",
      ...
      "legacy_checks": [
        {
          ...
          "min": 1,
          "max": 72,
          "_comment": "sStpCallback2: written every frame; always 0 in v1.5.2 (past its data end)"
        },
        {
          "addr": "0x0203CF90",
          "min": 0,
...
function identifyPatch() {
    ...
    if (magic === PATCH_MAGIC) {
        ...
        if (entry && entry.anchored) {
            if (!_identify_logged) {
                console.log("emerald_ne mapper: identified patch " + v + " (anchor)");
        ...
        if (!_identify_logged) {
            console.log(
                "emerald_ne mapper: ROM reports patch version " + v +
    ...
    for (const v of legacyVersions) {
        ...
        if (checkLegacyEntry(entry)) {
            if (!_identify_logged) {
                console.log("emerald_ne mapper: identified patch " + v + " (legacy)");
                _identify_logged = true;
            }
            return { version: v, entry: entry };
        }
...
export function preprocessor() {
    ...
    // Due to how the player data works for emerald, a pointer is stored in
    // IWRAM which tells us where the (DMA-shuffled) save data is located in
    // EWRAM, so we grab the pointers every loop.
