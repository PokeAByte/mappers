...
function containerprocessor(container, containerBytes) {
    ...
    if (container.startsWith("player_party_structure_")) {
        ...
        address = memory.defaultNamespace.get_uint32_le(0x2106FAC) + 0xD2AC + (236 * slotIndex);  // probably the wrong address
    } else {
        return;
    }
...
function preprocessor() {
    ...
    // HACKY SOLUTION - prevent the mapper from crashing on reset, but we need a better solution in the future.
    let enemy_ptr = 0;
    try {
        enemy_ptr = memory.defaultNamespace.get_uint32_le(base_ptr + 0x364C8); // Only needs to be calculated once per loop
    } catch (e) {
        enemy_ptr = 0;
    }
    // Set property values
    ...
    for (let i = 0; i < partyStructures.length; i++) {
        ...
        const offsets = {
            player            : 0xD2AC,
            static_player     : 0x597D8 + (0x5B0 * 0),
            static_wild       : 0x597D8 + (0x5B0 * 1),
            static_opponent   : 0x597D8 + (0x5B0 * 1),
            static_ally       : 0x597D8 + (0x5B0 * 2),
            static_opponent_2 : 0x597D8 + (0x5B0 * 3),
            dynamic_player    : 0x597D8 + (0x5B0 * 0),
            dynamic_opponent  : 0x597D8 + (0x5B0 * 1),
            dynamic_ally      : 0x597D8 + (0x5B0 * 2),         // TODO: Requires testing
            dynamic_opponent_2: 0x597D8 + (0x5B0 * 3),         // TODO: Requires testing
        };
